...
# Two options, the simple one or the Alpha-zero based big one:
from .AwariNNet import AwariNNet as onnet
#from .AwariNNet import ResNet as onnet
...
class NNetWrapper(NeuralNet):
    def __init__(self, game):
        ...
        # Awari mods:
        self.board_x, self.board_y, self.board_stack = game.getBoardSize()
        self.game = game
    ...
    def train(self, examples):
        ...
        if self.log_summary:
            ...
            summary_writer = tf.summary.FileWriter(FLAGS_log_dir + ('/train/iter-%02d' % self.global_iter), self.nnet.graph)
        self.global_iter += 1
        for epoch in range(args.epochs):
            ...
            while batch_idx < int(len(examples)/args.batch_size):
                ...
                if self.log_summary:
                    ...
                    summary_str, _ = self.sess.run([self.nnet.summary_merged, self.nnet.train_step], feed_dict=input_dict, options=run_options, run_metadata=run_metadata)
                    summary_writer.add_run_metadata(run_metadata, 'step%05d' % self.global_batch_idx)
                    summary_writer.add_summary(summary_str, self.global_batch_idx)
                else:
